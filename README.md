...
graph TB
    ...
    subgraph "Applications"
        CLI1[CLI-Anything Tools]
        CLI2[cli-anything-gimp]
        CLI3[cli-anything-blender]
    ...
    E1 -->|subprocess + --json| CLI1
    E1 --> CLI2
    E1 --> CLI3

    E2 -->|HTTPS + OAuth2| WEB1
    E2 --> WEB2
    E2 --> WEB3

    E3 -->|stdio JSON-RPC| ACP1
    E3 --> ACP2
    E3 --> ACP3

    E4 -->|Apple Events/DBus/COM| DESK1
    E4 --> DESK2

    D1 -.->|scan PATH| CLI1
    D2 -.->|fetch .well-known| WEB1
    D3 -.->|which command| ACP1
    D4 -.->|scan /Applications| DESK1
```
...
```
1. AAI Gateway scans PATH for cli-anything-* commands at startup
   → Found tools appear as app:<tool-id> entries in tools/list
2. User: "Use GIMP to resize this image"
   → Agent finds matching app:cli-anything.gimp
3. tools/call("app:cli-anything.gimp")
   → Returns operation guide: project/new(), image/resize()
4. tools/call("aai:exec", {app: "cli-anything.gimp", tool: "image/resize", args: {width: 800}})
   → Executes CLI command and returns JSON result
```

> 💡 **Note**: CLI tools are auto-discovered from commands matching `cli-anything-*` pattern in your PATH. Each tool must support `--aai` flag to output its descriptor.
...
| **CLI-Anything**  | CLI Tool  | None           | -     | Auto-discovered CLI tools generated by CLI-Anything |
...
There are three ways to integrate an app with AAI Gateway:
...
#### Method 2: CLI Tool (Auto-Discovery)

For CLI tools, name your command with `cli-anything-` prefix and implement `--aai` flag:

```bash
# Your CLI tool should respond to --aai flag
cli-anything-mytool --aai
# Output: AAI descriptor JSON

# Support --json for structured output
cli-anything-mytool --json some-command --arg value
# Output: JSON result
```

**CLI Tool Descriptor Example:**
...
CLI-Anything generated tools are automatically supported!

#### Method 3: Contribute to Built-in Registry
