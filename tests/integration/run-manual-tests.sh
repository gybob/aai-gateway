...
test_discovery() {
    ...
    # 测试 2.1: 基本扫描
    run_test "基本扫描" "node $CLI_BIN scan" 0

    # 测试 2.2: 开发模式扫描
    run_test "开发模式扫描" "node $CLI_BIN scan --dev" 0

    # 测试 2.3: 刷新扫描
    run_test "刷新扫描" "node $CLI_BIN scan --refresh" 0
}
...
test_compatibility() {
    ...
    run_test "TypeScript 类型检查" "cd $PROJECT_DIR && npm run typecheck" 0

    # 测试 4.2: ESLint 检查
    run_test "ESLint 检查" "cd $PROJECT_DIR && npm run lint" 0
}
...
test_unit_tests() {
    ...
    # 运行单元测试
    run_test "所有单元测试" "cd $PROJECT_DIR && npm test" 0
}
...
- **项目版本**: $(cd $PROJECT_DIR && node -p "require('./package.json').version")
...
### 项目信息
\`\`\`
$(cd $PROJECT_DIR && npm list --depth=0)
\`\`\`
...
main() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}AAI Gateway 集成测试${NC}"
    echo -e "${BLUE}========================================${NC}"
