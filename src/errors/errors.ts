...
export enum ErrorCode {
  ...
  /** OAuth/auth required — user must authorize before proceeding */
  AUTH_REQUIRED = -32011,
  /** Token expired and could not be refreshed */
  AUTH_EXPIRED = -32012,
  /** Web API request failed (4xx/5xx) */
  API_REQUEST_FAILED = -32013,
  /** Auth configuration is invalid or missing credentials */
  AUTH_CONFIG_INVALID = -32014,
}
...
export const ErrorType: Record<ErrorCode, string> = {
  ...
  [ErrorCode.AUTH_CONFIG_INVALID]: 'AUTH_CONFIG_INVALID',
};
...
export const ErrorMessage: Record<ErrorCode, string> = {
  ...
  [ErrorCode.AUTH_REQUIRED]: 'Authorization required',
  [ErrorCode.AUTH_EXPIRED]: 'Authorization expired',
  [ErrorCode.API_REQUEST_FAILED]: 'API request failed',
  [ErrorCode.AUTH_CONFIG_INVALID]: 'Invalid auth configuration',
};
...
export const Errors = {
  ...
  authRequired: (appId: string, detail?: string): AutomationError =>
    new AutomationError(
      ErrorCode.AUTH_REQUIRED,
      detail ?? `Authorization required for '${appId}'`
    ),

  authExpired: (appId: string): AutomationError =>
    new AutomationError(ErrorCode.AUTH_EXPIRED, `Token expired for '${appId}'`),

  apiRequestFailed: (appId: string, status: number, detail?: string): AutomationError =>
    new AutomationError(
      ErrorCode.API_REQUEST_FAILED,
      `API request to '${appId}' failed with status ${status}${detail ? `: ${detail}` : ''}`
    ),

  authConfigInvalid: (appId: string, detail: string): AutomationError =>
    new AutomationError(
      ErrorCode.AUTH_CONFIG_INVALID,
      `Invalid auth configuration for '${appId}': ${detail}`
    ),
};
